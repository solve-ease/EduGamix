      ...
      <div className="p-6">
        ...
        <div className="mb-6">
          <div className="flex justify-between mb-2">
            ...
            <span>{userData.achievements.total} / {userData.achievements.totalPossible} Earned</span>
          </div>
          <div className="w-full bg-gray-200 rounded-full h-2">
            <div 
              className="bg-blue-500 h-2 rounded-full" 
              style={{ width: `${(userData.achievements.total / userData.achievements.totalPossible) * 100}%` }}
            ></div>
          </div>
        </div>
            ...
            return (
              <div key={key} className="mb-4">
                <div className={`flex justify-between mb-2 p-2 rounded ${
                  key === 'avatarLevels' ? 'bg-green-500 text-white' : 
                  key === 'typingSpeed' ? 'bg-blue-500 text-white' :
                  key === 'accuracyStars' ? 'bg-blue-400 text-white' :
                  key === 'typingTests' ? 'bg-blue-500 text-white' :
                  key === 'timeSpent' ? 'bg-green-500 text-white' :
                  'bg-green-500 text-white'
                }`}>
                  <h3 className="font-bold">{title}</h3>
                  <span>{value.earned} / {value.total} Earned</span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div 
                    className="bg-blue-500 h-2 rounded-full" 
