...
const ProfilePage = () => {
  ...
  const [userData, setUserData] = useState({
    ...
    recentAchievements: [
      ...
      { 
        type: "time", 
        title: "80 WPM (3:00 Test)", 
        date: "a year ago", 
        icon: "hourglass" 
      }
    ],
    tasks: [
      { id: 1, title: "Complete JavaScript Basics", deadline: "March 15", type: "course" },
      { id: 2, title: "Frontend Mockup for Client", deadline: "March 18", type: "freelance" },
      { id: 3, title: "Team Code Review", deadline: "March 20", type: "job" }
    ],
    friends: [
      { id: 1, name: "Sarah Chen", skill: "UX Design", avatarLevel: "Tree" },
      { id: 2, name: "Marcus Lee", skill: "Backend Dev", avatarLevel: "Sapling" },
      { id: 3, name: "Priya Sharma", skill: "Data Science", avatarLevel: "Old Growth" }
    ],
    courses: [
      { id: 1, title: "Advanced React Patterns", progress: 65, totalLessons: 12, completedLessons: 8 },
      { id: 2, title: "Data Structures & Algorithms", progress: 30, totalLessons: 20, completedLessons: 6 },
      { id: 3, title: "UI/UX Fundamentals", progress: 90, totalLessons: 10, completedLessons: 9 }
    ]
  });
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-gray-100">
      ...
      <div className="flex-1 p-4">
        {/* Header Stats */}
        <HeaderStats stats={userData.stats} avatarLevel={userData.avatarLevel} avatarProgress={userData.avatarProgress} />
        
        {/* Main Content Area */}
        <div className="bg-white rounded-lg shadow-md mt-4 overflow-hidden">
          {/* Content based on active sidebar item */}
          {activeSidebarItem === 'summary' && <SummaryContent userData={userData} />}
          {activeSidebarItem === 'avatarLevels' && <AvatarLevelsContent userData={userData} />}
          {activeSidebarItem === 'tasks' && <TasksContent userData={userData} />}
          {activeSidebarItem === 'friends' && <FriendsContent userData={userData} />}
          {activeSidebarItem === 'courses' && <CoursesContent userData={userData} />}
          {activeSidebarItem === 'badges' && <BadgesContent userData={userData} />}
          {activeSidebarItem === 'timeSpent' && <TimeSpentContent userData={userData} />}
        </div>
      </div>
    </div>
  );
